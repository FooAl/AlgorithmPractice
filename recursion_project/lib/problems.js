...
function lucasNumber(n, memo = {}) {
    if(n === 0){
        memo = Object.assign(memo, {0: 2});
        return 2;
    }
    if(n === 1){
        memo = Object.assign(memo, {0: 2, 1: 1});
        return 1;
    }
    lucasNumber(n-1, memo);
...
function reverseString(str) {
    ...
    let reverseStr = last + reverseString(str.slice(0, -1));
    return reverseStr;
}
...
function pow(base, exponent) {
    ...
    if(exponent > 0){
        ...
        power = 1 / base * pow(base, exponent + 1);
    }
    return power;
}
...
function fileFinder(directories, targetFile) {
    let subDirectories = Object.keys(directories);
    for(let i = 0; i < subDirectories.length; i++){
        if(subDirectories[i].startsWith("/")){
            if(fileFinder(directories[subDirectories[i]], targetFile)){
                return true;
            }
        }else{
            if(subDirectories[i] === targetFile){
                return true;
            }
        }
    }
...
function pathFinder(directories, targetFile) {
    ...
    for(let i = 0; i < subDirectories.length; i++){
        if(subDirectories[i].startsWith("/")){
            let subPath = pathFinder(directories[subDirectories[i]], targetFile, pathName);
            if(subPath){
                pathName += subDirectories[i] + (subPath);
                return pathName;
            }
        }else{
            if (subDirectories[i] === targetFile) {
