...
function maxNonAdjacentSumTable(nums) {
    if(nums.length === 0) return 0;

    let table = new Array(nums.length).fill(0);
    table[0] = nums[0];
    for(let i = 1; i < table.length; i++){
        let skipLeftNeighbor;
        if(table[i-2] !== undefined){
            skipLeftNeighbor = table[i - 2];
        }else{
            skipLeftNeighbor = 0;
        }
        let maxCurrentIndex = skipLeftNeighbor + nums[i];
        let previousIndex = table[i-1];
        table[i] = Math.max(maxCurrentIndex, previousIndex);
    }
    return table[table.length -1];
}
...
function maxNonAdjacentSum(nums, memo = {}){
    ...
    let firstEle = nums[0];
    memo[nums.length] = Math.max(firstEle + maxNonAdjacentSum(nums.slice(2), memo), 
...
function minChange(coins, amount) {
    ...
    for(let i = 1; i < table.length; i++){
        ...
        for(let k = 0; k < coins.length; k++){
            let coin = coins[k];
            if(i - coin >= 0){
                if(table[i-coin] !== undefined){
                    paths.push(1 + table[i-coin]);
                }
            }
        }
        if(paths.length !== 0){
            table[i] = Math.min(...paths);
        }
    }
